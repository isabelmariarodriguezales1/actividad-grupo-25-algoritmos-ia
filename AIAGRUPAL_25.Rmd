...
Se ha seleccionado PCA porque permite resumir y explorar la estructura de los datos en un conjunto de alta dimensión, como es este caso.

Al mismo tiempo, el PCA es útil para explorar la estructura general de los datos, aunque no captura relaciones no lineales, por lo que se combina con ISOMAP para analizar patrones locales y complejos.
...
Se seleccionaron PCA y UMAP porque ofrecen visiones complementarias del dataset de expresión génica (alta dimensionalidad y posible estructura compleja):

PCA permite una reducción lineal de dimensionalidad maximizando la varianza explicada. En nuestro caso, el resultado muestra que se requieren aproximadamente 43 componentes para alcanzar el 70% de la varianza, lo que confirma la elevada complejidad y redundancia del dataset original. PCA es especialmente útil para detectar patrones globales y visualizar tendencias generales.

UMAP se seleccionó por su capacidad para capturar estructuras no lineales y preservar vecindarios locales, algo común en datos biológicos donde la separación entre grupos puede no ser lineal. UMAP genera una proyección bidimensional que facilita la visualización y suele ofrecer una mejor separación visual de clases que PCA en estos casos.
...
El rendimiento se evaluó mediante matriz de confusión y métricas (accuracy, sensibilidad, especificidad y F1). En este tipo de problemas multiclase, la métrica más informativa suele ser el F1 macro-promedio, ya que refleja el equilibrio del rendimiento entre clases.

En nuestro caso, los modelos más competitivos suelen ser SVM y Random Forest, ya que gestionan mejor la alta dimensionalidad y capturan patrones complejos. KNN también obtiene resultados razonables pero suele verse más limitado por la maldición de la dimensionalidad.
...
Sí, se consideró debido a la alta dimensionalidad del dataset. PCA/UMAP pueden reducir redundancia y ruido, y facilitar el entrenamiento de algunos modelos como KNN. Sin embargo, en este trabajo se priorizó:

filtrar genes no informativos (ceros/SD=0)

normalizar

usar modelos robustos en alta dimensión (SVM y RF)

Esto evita perder señal potencialmente útil al proyectar en componentes, y mantiene interpretabilidad de genes (especialmente importante en Random Forest para estudiar importancia de variables). PCA y UMAP se aplicaron principalmente en el bloque no supervisado como análisis exploratorio y de visualización.
